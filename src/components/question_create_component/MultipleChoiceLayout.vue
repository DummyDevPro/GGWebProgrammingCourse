        ...
        <div>
            ...
            <div class="input-group mb-1" v-for="(__, idx) in choice.items">
                ...
                <input type="text" v-model="choice.items[idx].val" :key="'txt' + choice.items[idx].key" class="form-control"
                    required>
                <button v-if="idx > 2" class="btn btn-outline-secondary" type="button"
                    @click="deleteOption(idx, choice.items[idx].key)">
...
export default {
    methods: {
        ...
        deleteOption(pos, key) {
            this.choice.items?.splice(pos, 1)

            // update ans keys
            const findIdxByOptKey = this.choice.ans.indexOf(key)
            if (findIdxByOptKey != -1) {
                this.choice.ans?.splice(findIdxByOptKey, 1)
            }
        }
    }
}
